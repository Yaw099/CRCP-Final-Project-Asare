...
public class MOrderMarkovGenerator<E> {
	...
	public SortTTOutputSequences sortTTSequences(ArrayList<ArrayList<E>> sequences, ArrayList<E> symbols, ArrayList<ArrayList<Float>> tt)	{
		...
		//use the current sorted lists to reference the counts & sequences and get the sorted counts
	...
	// round -- whether to round the probabilities to 2 decimal places (input) -- Training unit tests should be run with round = false, Generate, with round = true
	// sequences -- your list of sequences of symbols found (input)
	// symbols -- your alphabet or list of symbols (input)
	// tt -- the unsorted transition table -- NOTE!!!: expects **probabilities** NOT counts in the table (input)
	// mOrder -- the order of the markov chain (input)
	public void printProbabilityDistribution(boolean round, ArrayList<ArrayList<E>> sequences, ArrayList<E> symbols, ArrayList<ArrayList<Float>> tt, int mOrder)
	{
